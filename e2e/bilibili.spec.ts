...
test('gates, edits and recovers a B站 publication through the Electron UI', async () => {
  ...
  try {
    ...
    const partitionSelects = accountCard.locator('.partition-picker select')
    await expect(partitionSelects).toHaveCount(2)
    for (const select of await partitionSelects.all()) await expect(select).toBeDisabled()
    ...
    await expect(publishDialog.getByLabel('主分区')).toHaveValue('生活')
    await expect(publishDialog.getByLabel('子分区')).toHaveValue('21')
    ...
    await publishDialog.getByLabel('主分区').selectOption('动画')
    await expect(publishDialog.getByLabel('子分区')).toHaveValue('')
    await publishDialog.getByRole('button', { name: '确认投稿', exact: true }).click()
    await expect(publishDialog.getByRole('alert')).toHaveText('请选择投稿分区')
    await publishDialog.getByLabel('主分区').selectOption('生活')
    await publishDialog.getByLabel('子分区').selectOption('138')
    ...
    await expect(publishDialog.getByLabel(/标题/u)).toHaveValue('B站投稿 UI 验收')
    await expect(publishDialog.getByLabel('主分区')).toHaveValue('生活')
    await expect(publishDialog.getByLabel('子分区')).toHaveValue('138')
